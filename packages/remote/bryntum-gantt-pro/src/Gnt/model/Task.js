...
Ext.define('Gnt.model.Task', {
    ...
    /**
     * @cfg {String} forceParentSchedulingMode
     *
     * This configuration option defines scheduling mode that should be applied
     * to a task once it becomes a parent.
     * 
     * Provide `false` to disable that applying.
     */
    forceParentSchedulingMode   : 'Normal',
        ...
        if (!this.isRoot() && length > 0 && !this.isManuallyScheduled() && !this.isReadOnly()) {
            ...
            if (isProject) {
                if (scheduleBackwards) {
                    earliest = latest = this.getEndDate();
                } else {
                    earliest = latest = this.getStartDate();
                }
            }
    ...
    /**
     * @protected
     * The method is called once the task becomes a parent.
     * It sets the task "leaf" attribute to `false`, resets {@link #Segments} field
     * (since a parent task cannot be split) and sets 
     * {@link #SchedulingMode scheduling mode} to {@link #forceParentSchedulingMode} value if provided.
     * 
     * Override this method if you need some custom actions to happen when the task becomes a parent.
     */
    markAsParent : function() {
        ...
        if (me.forceParentSchedulingMode) {
            // since the task became a parent we switch its scheduling mode to 'Normal' (ticket #1441)
            me.set(me.schedulingModeField, me.forceParentSchedulingMode);
        }
    },
    ...
    setReadOnly : function (value, callback) {
        var me = this;

        me.propagateChanges(
            function() {
                return me.setReadOnlyWithoutPropagation(value);
            },
            callback
        );
    },

    setReadOnlyWithoutPropagation : function (state) {
        this.set(this.readOnlyField, state);
    ...
    // This hook is called in record.set()
    processFieldPreviousValue : function (field, value) {
        // store "Segments" field previous value properly
        if (field === this.segmentsField) {
            if (this.isCommittingProjection && this.segmentsProjection) {
                this.previousSegmentsValue = this.segmentsProjection[0];
            }
            else if (!this.isProjected(this.segmentsField)) {
                this.previousSegmentsValue = this.buildSegmentsSnapshot(value);
            }
        }
