...
Ext.define('Gnt.model.task.More', {
        ...
        if (taskStore) {
            if (cancelChanges) {
                ...
                for (internalId in affected) {
                    ...
                    if (!modifiedData || !modifiedData[internalId] || !modifiedData[internalId].length) {
                        nbrUpdated = true;
    ...
    isTwoStepPropagationNeeded : function (taskStore, propagationSources, project, oppositeTasks) {
        ...
        if (propagationSources[0].isContraryScheduled(taskStore, project)) {
            ...
            Ext.Array.each(propagationSources, function (source) {
                ...
                } else if (source.isContraryScheduled(null, project) && source.isProjected(source.readOnlyField)) {
                    result = true;
                    return false;
    ...
    /**
     * The method is called to turn this empty parent task into
     * a leaf task when {@link #convertEmptyParentToLeaf} is `true`.
     * 
     * Override the method to customize this process.
     * @protected
     */
    convertEmptyParentToLeafTask : function() {
        this.beginEdit();
        this.set('leaf', true);
        if (this.isEffortDriven()) {
