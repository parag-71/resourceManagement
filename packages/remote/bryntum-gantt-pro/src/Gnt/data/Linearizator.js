Ext.define('Gnt.data.Linearizator', function(thisClass) {
    ...
    var walkingSpecification, addTaskSuccessors, removeTaskSuccessors, addTaskPredecessors, removeTaskPredecessors;
    ...
    function linearWalkBySpecification(sourceTasksList, processorFn, walkingSpec) {
        ...
        var taskStore = sourceTasksList.getTaskStore ? sourceTasksList.getTaskStore(true) : sourceTasksList[0].getTaskStore(true);

        walkingSpecification   = walkingSpec;
        addTaskSuccessors      = {};
        removeTaskSuccessors   = {};
        addTaskPredecessors    = {};
        removeTaskPredecessors = {};

        if (taskStore) {
            if (walkingSpec.successors) {
                // If provided lists of dependencies planned to add/remove
                if (walkingSpec.addDependencies) {
                    Ext.each(walkingSpec.addDependencies, function (dep) {
                        var fromId = dep.getFrom ? dep.getFrom() : dep.From,
                            toId   = dep.getTo ? dep.getTo() : dep.To;

                        addTaskSuccessors[fromId] = addTaskSuccessors[fromId] || [];
                        addTaskSuccessors[fromId].push(taskStore.getModelById(toId));
                    });
                }
                if (walkingSpec.removeDependencies) {
                    Ext.each(walkingSpec.removeDependencies, function (dep) {
                        var fromId = dep.getFrom ? dep.getFrom() : dep.From,
                            toId   = dep.getTo ? dep.getTo() : dep.To;

                        removeTaskSuccessors[fromId] = removeTaskSuccessors[fromId] || [];
                        removeTaskSuccessors[fromId].push(taskStore.getModelById(toId));
                    });
                }
            }

            if (walkingSpec.predecessors) {
                // If provided lists of dependencies planned to add/remove
                if (walkingSpec.addDependencies) {
                    Ext.each(walkingSpec.addDependencies, function (dep) {
                        var fromId = dep.getFrom ? dep.getFrom() : dep.From,
                        ...
                        addTaskPredecessors[toId] = addTaskPredecessors[toId] || [];
                        addTaskPredecessors[toId].push(taskStore.getModelById(fromId));
                    });
                }
                if (walkingSpec.removeDependencies) {
                    Ext.each(walkingSpec.removeDependencies, function (dep) {
                        ...
                        removeTaskPredecessors[toId] = removeTaskPredecessors[toId] || [];
                        removeTaskPredecessors[toId].push(taskStore.getModelById(fromId));
                    });
                }
            }
        }
    ...
    function successorsTasksDepsCollector$(task, tasksColorMap$, depsColorMap$) {
        ...
        // If provided lists of dependencies planned to add/remove
        if (removeTaskSuccessors[task.id]) {
            successors = Ext.Array.difference(successors.slice(), removeTaskSuccessors[task.id]);
        }
        if (addTaskSuccessors[task.id]) {
            successors = Ext.Array.merge(successors, addTaskSuccessors[task.id]);
        }
    ...
    function predecessorsTasksDepsCollector$(task, tasksColorMap$, depsColorMap$) {
        ...
        // If provided lists of dependencies planned to add/remove
        if (removeTaskPredecessors[task.id]) {
            predecessors = Ext.Array.difference(predecessors.slice(), removeTaskPredecessors[task.id]);
        }
        if (addTaskPredecessors[task.id]) {
            predecessors = Ext.Array.merge(predecessors, addTaskPredecessors[task.id]);
        }
